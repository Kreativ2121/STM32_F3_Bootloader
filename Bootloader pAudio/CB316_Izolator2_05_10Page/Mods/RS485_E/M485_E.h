...
/******************** KLASY ***************************************************************/      
  ...
  class M485E{
    ...
    public: uint16_t addCrc( uint16_t crc, uint8_t b ) {
		uint8_t i;
		uint16_t c;
		c = ((crc >> 8) ^ b) << 8;
		i = 0;
    ...
    //Funkcja umożliwiająca wyliczenie crc dla gotowej ramki
    public: uint16_t crcToFrame( uint8_t* frame) {
    	uint16_t crc = 0;

    	for(uint8_t i = 0; i < 6; i++)
    	{
    		crc = this->addCrc(crc,frame[i]);
    	}

    	uint8_t sizeOfData = frame[5];
    	for(uint8_t i = 6; i < 6 + sizeOfData; i++)
    	{
    		crc = this->addCrc(crc,frame[i]);
    	}
    ...
    //Funkcja wyliczająca wartość CRC z otrzymanej ramki
    public: bool crcCheck( const uint8_t* frameBuf ) {
    	uint16_t crc = 0;
    	uint8_t sizeOfData = frameBuf[5];

    	for(uint8_t i = 0; i < 6; i++)
    	{
    		crc = this->addCrc(crc,frameBuf[i]);	//Obliczanie CRC z nagłówka
    	}

    	for(uint8_t i = 8; i < 8 + sizeOfData; i++)
    	{
    		crc = this->addCrc(crc,frameBuf[i]);	//Obliczanie CRC z danych
    	}

    	uint8_t crc_H = (crc >> 8);
    	uint8_t crc_L = (crc >> 0);

    	uint8_t crcOrg_H = frameBuf[6];
    	uint8_t crcOrg_L = frameBuf[7];

    	if((crcOrg_H == crc_H) && (crcOrg_L == crc_L))
    	{
    		return true;	//Jeżeli CRC się zgadza
    	}
    	else
    	{
    		return false;	//Jeżeli CRC jest błędne
    	}
	}//----------------------------------------
